...
def star_shape_metrics(
    ...
    # Get bounding boxes for all labels in one pass
    slices = ndi_find_objects(labels)
    if not slices:
        return np.array([]), np.array([]), np.array([]), np.array([])

    for lb in range(1, max_id + 1):
        sl = slices[lb - 1] if lb - 1 < len(slices) else None
        if sl is None:
            continue
        ysl, xsl = sl  # slice objects for y and x
        # Expand ROI by pad pixels within bounds (handle None starts/stops)
        y0e = max(0, (ysl.start or 0) - pad); y1e = min(img.shape[0], (ysl.stop or img.shape[0]) + pad)
        x0e = max(0, (xsl.start or 0) - pad); x1e = min(img.shape[1], (xsl.stop or img.shape[1]) + pad)
        ...
        ring = stamp[~msub]
        bg_local = np.median(stamp) if ring.size == 0 else np.median(ring)
        flux = stamp - bg_local
        flux[flux < 0] = 0.0
        ...
        evals = np.clip(np.sort(evals), 1e-12, None)
        sig_minor, sig_major = np.sqrt(evals[0]), np.sqrt(evals[1])

        fwhm_major = FWHM_FACTOR * sig_major
        fwhm_minor = FWHM_FACTOR * sig_minor
        fwhm = math.sqrt(max(1e-12, fwhm_major * fwhm_minor))
        fwhm_vals.append(fwhm)

        a = max(sig_major, sig_minor)
        b = min(sig_major, sig_minor)
        ...
        r_flat = r[msub].ravel()
        f_flat = w[msub].ravel()
        if f_flat.size == 0 or f_flat.sum() <= 0:
            continue
        ...
        if idx >= len(r_sorted):
            idx = len(r_sorted) - 1
        hfr_vals.append(float(r_sorted[idx]))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.sp_arcsec = QDoubleSpinBox(self); self.sp_arcsec.setDecimals(4); self.sp_arcsec.setRange(0.001, 10.0); self.sp_arcsec.setValue(0.0)
        self.sp_arcsec.setToolTip("Pixel scale in arcsec/pixel. If >0 and 'Report arcseconds' is checked, FWHM/HFR in arcsec are included.")
        self.chk_arcsec = QCheckBox("Report arcseconds", self); self.chk_arcsec.setChecked(False)
        self.chk_arcsec.setToolTip("Enable to compute and report FWHM/HFR in arcseconds using the pixel scale field.")
        ...
        arc_layout.addWidget(self.chk_arcsec)
        self.lbl_arcsec = QLabel("arcsec/px:", self); self.lbl_arcsec.setToolTip(self.sp_arcsec.toolTip())
        arc_layout.addWidget(self.lbl_arcsec)
        arc_layout.addWidget(self.sp_arcsec)
        arc_layout.addStretch()
        form.addRow(arc_layout)
    ...
    @Slot()
    def on_calc_scale(self):
        px_um = float(self.sp_px_um.value())
        fl_mm = float(self.sp_flen.value())
        binning = int(self.sp_binning.value())
        reducer = float(self.sp_reducer.value())

        eff_px_um = px_um * max(1, binning)
        eff_fl_mm = fl_mm * max(0.0001, reducer)
        scale = 206.265 * (eff_px_um / eff_fl_mm)

        self.sp_arcsec.setValue(scale)
        self.chk_arcsec.setChecked(True)
        self.progress.setFormat(f"Pixel scale set: {scale:.3f} arcsec/px")
    ...
    def _gather_params(self) -> Dict[str, Any]:
        arc = None
        if self.chk_arcsec.isChecked():
            val = float(self.sp_arcsec.value())
            if val > 0:
                arc = val
        return {
